...
public class DrawableTextView extends AppCompatTextView {
    ...
    public DrawableTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mRightListDrawable = (StateListDrawable) array.getDrawable(R.styleable.DrawableTextView_rightDrawableList);
        mRightListDrawableWidth = array.getDimensionPixelSize(R.styleable.DrawableTextView_rightDrawableListWidth, 0);
        mRightListDrawableHeight = array.getDimensionPixelSize(R.styleable.DrawableTextView_rightDrawableListHeight, 0);
        array.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mRightListDrawable != null) {
            int left = (int) (centerX + halfTextWidth + drawablePadding);
            int top = (int) (centerY - mRightListDrawableHeight / 2);
            mRightListDrawable.selectDrawable(isSelected() ? 0 : 1);
            BitmapDrawable drawable = (BitmapDrawable) mRightListDrawable.getCurrent();
            drawable.setBounds(
                    left,
                    top,
                    left + mRightListDrawableWidth,
                    top + mRightListDrawableHeight);
            canvas.save();
            Log.e("DrawableTextView", "----onDraw---" + isSelected());
    ...
    private void translateText(Canvas canvas, int drawablePadding) {
        ...
        if (mRightListDrawable != null) {
            translateWidth = -(mRightListDrawableWidth + drawablePadding) / 2;
        }
